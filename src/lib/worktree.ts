...
export async function getWorktreeStatus(
  ...
  try {
    ...
    try {
      const { stdout: staged } = await execGitCommandSync(
    ...
    try {
      const { stdout: modified } = await execGitCommandSync(
        ["diff", "--name-only"],
        worktreePath,
      );
    ...
    try {
      const { stdout: untracked } = await execGitCommandSync(
    ...
    try {
      const { stdout: upstream } = await execGitCommandSync(
        ["rev-list", "--left-right", "--count", "@{u}...HEAD"],
        worktreePath,
      );
      const [incoming, outgoing] = upstream
        .trim()
        .split("\t")
        .map((n) => parseInt(n) || 0);
