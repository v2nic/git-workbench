...
export async function POST(request: NextRequest) {
  try {
    const {
      ...
      newBranchName,
      worktreeName,
    }: CreateFromBranchRequest = await request.json();
    ...
    const repoConfig = config.repos.find(
      (r) => r.fullName === repo || r.repoName === repo,
    );
    ...
    await execCommand(
      `git --git-dir "${barePath}" branch "${newBranchName}" "${fromBranch}"`,
    );

    // Create worktree for the new branch
    await execCommand(
      `git --git-dir "${barePath}" worktree add "${worktreePath}" "${newBranchName}"`,
    );
    ...
    try {
      ...
      if (fromBranch.startsWith("origin/")) {
        ...
        await execCommand(
          `git -C "${worktreePath}" branch --set-upstream-to="${fromBranch}" "${newBranchName}"`,
        );
        await execCommand(
          `git -C "${worktreePath}" config push.default upstream`,
        );
      } else {
        ...
        await execCommand(
          `git -C "${worktreePath}" config branch."${newBranchName}".pushRemote origin`,
        );
        await execCommand(
          `git -C "${worktreePath}" config branch."${newBranchName}".remote origin`,
        );
        await execCommand(
          `git -C "${worktreePath}" config branch."${newBranchName}".merge refs/heads/${newBranchName}`,
        );

        // Set upstream to the local branch for pull operations if it exists remotely
        const upstreamExists = await execCommand(
          `git -C "${worktreePath}" rev-parse --verify "origin/${fromBranch}"`,
        )
          .then(() => true)
          .catch(() => false);
        if (upstreamExists) {
          await execCommand(
            `git -C "${worktreePath}" branch --set-upstream-to="origin/${fromBranch}" "${newBranchName}"`,
          );
        }
      }
