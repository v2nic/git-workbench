...
export async function POST(request: NextRequest) {
  try {
    const { repoFullNameOrName, worktreeName, ref }: CreateWorktreeRequest =
      await request.json();
    ...
    let repoConfig = config.repos.find(
      (r) =>
    ...
    if (
      !config.repos.find(
        (r) =>
          r.repoName === repoConfig.repoName ||
          r.fullName === repoConfig.fullName,
      )
    ) {
      wasUntracked = true;
      try {
        ...
        console.log(
          `Added untracked repository '${repoConfig.repoName}' to configuration`,
        );
      } catch (configError) {
        console.error(
    ...
    const refExists = async (
      ...
      try {
        await execCommand(
          `git --git-dir "${dir}" rev-parse --verify "${refName}"`,
        );
        return true;
      } catch {
        return false;
    ...
    if (!(await refExists(gitDir, targetRef))) {
      ...
      if (headBranch !== targetRef && (await refExists(gitDir, headBranch))) {
        console.log(
          `Target ref "${targetRef}" not found, falling back to HEAD branch "${headBranch}"`,
        );
    ...
    try {
      await execCommand(
        `git --git-dir "${gitDir}" rev-parse --verify "refs/heads/${worktreeName}"`,
      );
      branchExists = true;
    } catch {
    ...
    if (branchExists) {
      ...
      await execCommand(
        `git --git-dir "${gitDir}" worktree add "${worktreePath}" "${worktreeName}"`,
      );
    } else {
      // Branch doesn't exist, create it from the starting point
      await execCommand(
        `git --git-dir "${gitDir}" worktree add -b "${worktreeName}" "${worktreePath}" "${targetRef}"`,
      );
    }
    ...
    try {
      ...
      const upstreamExists = await execCommand(
        `git -C "${worktreePath}" rev-parse --verify "origin/${worktreeName}"`,
      )
        .then(() => true)
        .catch(() => false);
      if (upstreamExists) {
        await execCommand(
          `git -C "${worktreePath}" branch --set-upstream-to="origin/${worktreeName}" "${worktreeName}"`,
        );
      } else {
        // Branch doesn't exist remotely, configure push to go to origin/worktreeName by default
        await execCommand(
          `git -C "${worktreePath}" config push.default current`,
        );
        await execCommand(
          `git -C "${worktreePath}" config branch."${worktreeName}".pushRemote origin`,
        );
        await execCommand(
          `git -C "${worktreePath}" config branch."${worktreeName}".remote origin`,
        );
        await execCommand(
          `git -C "${worktreePath}" config branch."${worktreeName}".merge refs/heads/${worktreeName}`,
        );
        ...
        if (
          ...
          targetRef !== `origin/${worktreeName}`
        ) {
          await execCommand(
            `git -C "${worktreePath}" branch --set-upstream-to="${targetRef}" "${worktreeName}"`,
          );
        }
      }
