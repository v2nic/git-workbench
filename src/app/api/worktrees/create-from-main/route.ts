...
export async function POST(request: NextRequest) {
  try {
    const { repo, newBranchName, worktreeName }: CreateFromMainRequest =
      await request.json();
    ...
    await execCommand(
      `git --git-dir "${barePath}" branch "${newBranchName}" "origin/${defaultBranch}"`,
    );
    ...
    try {
      ...
      await execCommand(
        `git -C "${worktreePath}" branch --set-upstream-to="origin/${defaultBranch}" "${newBranchName}"`,
      );
