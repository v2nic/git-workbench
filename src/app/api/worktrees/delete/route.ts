...
function compareStatus(
  ...
  if (expected.staged !== actual.staged) {
    ...
    expected.stagedHash &&
    actual.stagedHash &&
    expected.stagedHash !== actual.stagedHash
  ) {
    mismatches.push({
      type: "staged",
      expected: expected.staged,
  ...
  if (expected.modified !== actual.modified) {
    ...
    expected.modifiedHash &&
    actual.modifiedHash &&
    expected.modifiedHash !== actual.modifiedHash
  ) {
    mismatches.push({
      type: "modified",
      expected: expected.modified,
  ...
  if (expected.untracked !== actual.untracked) {
    const newFiles =
      actual.untrackedFiles && expected.untrackedFiles
        ? actual.untrackedFiles.filter(
            (f) => !expected.untrackedFiles!.includes(f),
          )
        : [];
    ...
    expected.untrackedHash &&
    actual.untrackedHash &&
    expected.untrackedHash !== actual.untrackedHash
  ) {
    mismatches.push({
      type: "untracked",
      expected: expected.untracked,
...
export async function POST(request: NextRequest) {
  try {
    const { repo, worktreePath, force, expectedStatus }: DeleteWorktreeRequest =
      await request.json();
    ...
    await execCommand(
      `git --git-dir "${barePath}" worktree remove ${forceFlag} "${worktreePath}"`,
    );
