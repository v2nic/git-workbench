...
export function usePullRequests() {
  ...
  const checkForRemovedPRs = useCallback((newPRs: PRNotification[]) => {
    ...
    const hasRemovals = Array.from(currentUrls).some(
      (url) => !newUrls.has(url),
    );

    // Check if any PRs were added
    const hasAdditions = Array.from(newUrls).some(
  ...
  useEffect(() => {
    ...
    const onSnapshot = (evt: MessageEvent<string>) => {
      ...
      prsByUrlRef.current = new Map(
        data.pullRequests.map((pr) => [pr.url, pr]),
      );
      ...
      if (
        data.status?.type === "rate_limited" &&
        typeof data.status.retryAfterMs === "number"
      ) {
        setRateLimited(true);
    ...
    const onStatus = (evt: MessageEvent<string>) => {
      const data = JSON.parse(evt.data) as {
        status: { type: string; retryAfterMs?: number; message?: string };
        asOf: number;
      };
      setTimestamp(data.asOf);
