...
export function WorktreesView({
  ...
  const [highlightWorktreeName, setHighlightWorktreeName] = useState<
    string | undefined
  >();
  ...
  useEffect(() => {
    ...
    const targetWorktree = worktrees.find(
      (worktree) => getWorktreeName(worktree) === highlightWorktreeName,
    );
  ...
  const worktreesByRepo = useMemo(() => {
    ...
    if (filterRepo) {
      ...
      repos.forEach((repo) => {
        const matchesFilter =
          repo.repoName === filterRepo || repo.fullName === filterRepo;
    ...
    if (filterRepo) {
      entries = entries.filter(([repoName]) => {
        ...
        return (
          repo && (repo.repoName === filterRepo || repo.fullName === filterRepo)
        );
      });
    }
  ...
  const handleDeleteWorktree = useCallback(
    async (worktree: Worktree) => {
      try {
        ...
        mutate();
        onSuccess?.(
          `Worktree '${worktree.pathRelativeToHome}' deleted successfully`,
        );
      } catch (error) {
  ...
  const handleForceDeleteWithMismatch = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/worktrees/delete", {
        ...
        body: JSON.stringify({
          repo:
            pendingDeleteWorktree.repoFullName ||
            pendingDeleteWorktree.repoName,
      ...
      mutate();
      onSuccess?.(
        `Worktree '${pendingDeleteWorktree.pathRelativeToHome}' deleted successfully`,
      );
    } catch (error) {
      console.error("Failed to delete worktree:", error);
      onError?.(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (error.mismatches) {
          ...
          const mismatchDetails = error.mismatches
            .map((m: any) => {
          ...
          onError?.(
            `Warning: Worktree state changed during deletion: ${mismatchDetails}. Proceeding anyway.`,
          );
        } else {
          throw new Error(error.error || "Failed to delete worktree");
        }
        ...
        {isLoading ? (
          ...
          <div className="p-6 text-center">
            <p className="text-muted-foreground">
              {filterRepo
                ? `No repository found matching ${filterRepo}`
                  ...
                  <div>
                    {repoWorktrees.length === 0 ? (
                      <div className="p-4 text-center text-muted-foreground">
                        <p className="text-sm">
                          No worktrees yet. Create your first worktree above!
                        </p>
                      </div>
                    ) : (
                      <div>
