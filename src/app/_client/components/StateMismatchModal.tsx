      ...
      if (mismatch.type === "untracked" && mismatch.newFiles) {
        return (
          <div key={index} className="text-red-600 dark:text-red-400">
            <span className="font-medium">Untracked files:</span>{" "}
            {mismatch.newFiles.length} new file(s)
            <div className="text-xs mt-1 font-mono opacity-75">
              {mismatch.newFiles.slice(0, 5).join(", ")}
              {mismatch.newFiles.length > 5 &&
                `... and ${mismatch.newFiles.length - 5} more`}
            </div>
          </div>
        );
        ...
        return (
          <div key={index} className="text-orange-600 dark:text-orange-400">
            <span className="font-medium">{mismatch.type} files:</span> File
            list changed (count unchanged)
          </div>
        );
      } else {
        return (
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1">
            <p className="text-sm text-foreground">
              ...
              has changed since you viewed it:
            </p>
          </div>
        </div>
